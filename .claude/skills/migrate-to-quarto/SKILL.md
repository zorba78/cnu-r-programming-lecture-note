...
**장 제목은 YAML `title:` 이 맡고, 본문은 `##` (절) 부터 시작한다.**
YAML `title:` 과 본문 `# 장제목` H1 을 **둘 다 두면 안 된다.** 제목이 두 번 출력되고,
오른쪽 목차의 최상위가 그 H1 하나로 묶여 절 전체가 한 단계 안으로 접혀 들어간다
(`toc-expand` 를 주기 전에는 목차에 장 제목 한 줄만 보인다). 절이 최상위로 오면
번호도 `1, 1.1, 1.2` 로 깔끔해진다.
...
각 장 YAML 은 `toc-depth: 2` 로 두어 **오른쪽 목차에 절까지만** 싣는다. 소절까지 실으면
8장 기준 29줄이 되어 오히려 찾기 어렵다. `toc-expand` 로 "읽는 절만 펼치기"를 시도하면
스크롤을 내려도 두 번째 이후 절이 펼쳐지지 않아 그 소절에 목차로 닿을 수 없으니 쓰지 않는다.
본문 쪽은 **모든 절을 소절로 나눈다.** 소절 없이 100줄 넘게 흐르는 절을 만들지 않는다.
