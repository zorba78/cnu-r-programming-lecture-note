...
```{r, eval=FALSE, comment=NA, tidy=TRUE}
...
```{r multiple-package, eval=TRUE, warning=FALSE}
require(tidyverse)
```
...
```{r assign-diff, comment=NA, error=TRUE}
# mean(): 입력 벡터의 평균 계산
mean(y <- 1:5)
y
mean(x = 1:5)
...
```{r objectName-ex01, echo = T, eval = T, prompt = F}
...
```{r objName-ex02, comment=NA, error=TRUE}
3x <- 7
```

```{r objName-ex03, comment=NA, error=TRUE}
_x <- c("M", "M", "F")
```

```{r objName-ex04, comment=NA, error=TRUE}
.3 <- 10
```
