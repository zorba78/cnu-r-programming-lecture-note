...
```{block2, type="rmdnote"}
**학습목표**: R에서 사용 가능한 데이터 타입에 대해 알아보고, 고유 데이터 타입으로 구성한 객체(스칼라, 백터, 리스트)와 이와 연관된 함수들을 익힌다.
```
...
```{r rmarkdown-part, fig.align='center', echo=FALSE, fig.show="hold", out.width='100%', fig.cap=cap}
cap <- "R 데이터 타입 구조 다이어그램: [R, Python 분석과 프로그래밍 (by R Friend)]( http://rfriend.tistory.com/)에서 발췌 후 수정"
...
kable(tab2_03,
  ...
  column_spec(1, width = "3cm") %>% 
  column_spec(2, width = "7cm") %>% 
  kableExtra::footnote(general = "기술한 비교 연산자는 수치형 및 논리형 데이터 타입 모두에 적용 가능 하지만, 문자형 비교 연산은 ==, != 만 가능함", 
                       threeparttable = TRUE) #%>% 
...
```{r ex-step4, comment=NA, fig.align='center', echo=TRUE, fig.show='hold', out.width='90%'}
ggdraw() +
  draw_image(pic)
...
```{r, echo = F}
`이름` <- c("라파엘 나달", "다닐 메드베데프", "노박 조코비치", "권순우")
`국적` <- c("스페인", "러시아", "세르비아", "대한민국")
`나이` <- c(35, 25, 34, 25)
`순위` <- c(6, 2, 1, 53)
tab <- data.frame(`이름`, `국적`, `나이`, `순위`)
