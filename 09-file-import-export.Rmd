...
read.table(
  ...
  stringsAsFactors = FALSE, # 문자형 변수를 factor로 변환할 지 여부
                            # R 4.0.0부터 기본값이 FALSE로 변경됨
...
```{block2, type="rmdcaution"}
R 4.0.0 이전에는 `read.table()`이 문자형 값을 자동으로 요인(factor)으로 변환했고, 이를 막으려면 `stringsAsFactors = FALSE`를 명시해야 했다. R 4.0.0부터 기본값이 `FALSE`로 바뀌어, 이제는 반대로 **요인으로 변환하고 싶을 때** `stringsAsFactors = TRUE`를 지정한다. 오래된 교재나 인터넷 예제에서 `stringsAsFactors = FALSE`가 반복적으로 등장하는 이유가 여기에 있다.
```
