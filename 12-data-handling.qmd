...
부푼다. 코드는 오류 없이 끝나고 표는 그럴듯하다. 틀렸다는 사실은 **무엇이 보존되어야 하는지** 알고
...
321,746권만큼 많다. 표를 위에서부터 읽으면 1위와 2위가 같은 값이지만, 순위표에서는
"공동 1위"처럼 보여 지나치기 쉽다.
...
좋은 응답이었다. 처리 순서를 설명했고, 확인 코드를 넣었고, 휴관한 달을 0 으로 보는 가정이 **결정**이라는
